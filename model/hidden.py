...
class Hidden:
    def __init__(self, configuration: HiDDenConfiguration, device: torch.device, noiser: Noiser, tb_logger):
        ...
        if configuration.use_vgg:
            ...
            # 3 blocks and each has 1 layer and do not use batch_norm vgg
            self.vgg_loss.to(device)
        else:# default use_vgg=false
            self.vgg_loss = None
        ...
        if tb_logger is not None:
            ...
            #register_hook的作用主要是得到梯度下降的中间变量，用来跟踪收敛的，register_hook针对的是variable
    ...
    def train_on_batch(self, batch: list):
        ...
        with torch.enable_grad():
            ...
            #初始化所有矩阵为数值都是1的单位矩阵，大小是batch-size的大小
            ...
            #binary cross entropy loss with logits（aka with sigmoid） 算损失函数
            d_loss_on_cover.backward()#compute gradients but do not change D's parameters
            ...
            #optimize D's parameters;changes based on stored gradients from backward()
